...
public class ServiceCodesCostsResponse
{
   public ServiceCodesCostsResponse(ServiceCodesCosts costsRecords)
   {
      ...
      ByServiceCode = costsRecords.GetCostsByServiceCodes().ToDictionary(kvp => kvp.Key, kvp => kvp.Value.Select(costs => new ServiceCodeCostReportResponse(costs)).ToList());
      ByEnvironment = costsRecords.GetCostsByEnvironments().ToDictionary(kvp => kvp.Key, kvp => kvp.Value.Select(costs => new ServiceCodeCostReportResponse(costs)).ToList());
   }
