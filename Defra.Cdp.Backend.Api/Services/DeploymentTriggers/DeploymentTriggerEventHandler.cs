    ...
    ILogger<DeploymentTriggerEventHandler> logger)
{
   ...
   {
      logger.LogInformation("{id} Handling EcsDeploymentStateChange trigger test runs for {deploymentId}, {name} {reason}",
       id, ecsEvent.Detail.DeploymentId, ecsEvent.Detail.EventName, ecsEvent.Detail.Reason);

      var deployment = await deploymentsService.FindDeploymentByLambdaId(ecsEvent.Detail.DeploymentId, cancellationToken);
      if (deployment == null)
      {
      ...
      {
         var deploymentTriggers = await deploymentTriggerService.FindTriggersForDeployment(deployment, cancellationToken);

         foreach (var trigger in deploymentTriggers)
         {
            logger.LogInformation("{id} Triggering test run for {deploymentId} {testSuite}", id, ecsEvent.Detail.DeploymentId, trigger.TestSuite);

            await selfServiceOpsFetcher.TriggerTestSuite(trigger.TestSuite, deployment.Environment, deployment.User, cancellationToken);
