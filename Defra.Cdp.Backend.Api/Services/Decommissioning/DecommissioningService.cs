    ...
    ) : IJob 
{
    ...
    private readonly ILogger<DecommissioningService> _logger =
        loggerFactory.CreateLogger<DecommissioningService>();
    ...
    public async Task Execute(IJobExecutionContext context)
    {
        ...
        {
            _logger.LogInformation("Decommissioning entity {EntityName} in status {Status}", entity.Name, entity.Status);
            var deployments = await deploymentsService.RunningDeploymentsForService(entity.Name, context.CancellationToken);
            ...
            if (!entityStatus!.Entity.Decommissioned.WorkflowsTriggered)
            {
                ...
                await entitiesService.DecommissioningWorkflowsTriggered(entity.Name, context.CancellationToken);
                     ...
                     && await deployableArtifactsService.FindAllTagsForRepo(entity.Name, context.CancellationToken) is { Count: 0 }
                     )
            {
            ...
            {
                _logger.LogWarning("Entity {EntityName} has resources that are not ready for decommissioning.", entity.Name);
            }
        }
    }
}