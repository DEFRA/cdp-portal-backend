    ...
    ILogger<TriggerWorkflowService> logger) : ITriggerWorkflowService
{
    ...
    public async Task<GitHubTriggerWorkflowResponse?> TriggerWorkflow<T>(string repo, string workflow, T inputs,
        CancellationToken cancellationToken) where T : IGithubWorkflowInputs
    {
        ...
        logger.LogInformation("Triggering workflow {Workflow} in repo {Repo} with payload {Payload}", workflow, repo,
            jsonPayload);
