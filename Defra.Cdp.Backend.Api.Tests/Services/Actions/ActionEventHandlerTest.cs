...
{
    [Fact]
    public async Task WillProcessAndSaveMessage()
    {
        var service = Substitute.For<IAppConfigVersionService>();
        ...
        var mockPayload = ActionEventHandler.TryParseMessageBody(
            @"{""action"": ""app-config-version"", ""content"" : {""commitSha"":""abc123"", ""commitTimestamp"":""2024-10-23T15:10:10.123"", ""environment"":""infra-dev""}}");
    ...
    {
        var body =
            @"{""action"": ""some-action"", ""content"" : {""json"": ""anything""}}";
    ...
    public void CanDeserialiseBody()
    {
        ...
        var parsedBody = messageHeader.Content.Deserialize<AppConfigVersionMessageContent>();
