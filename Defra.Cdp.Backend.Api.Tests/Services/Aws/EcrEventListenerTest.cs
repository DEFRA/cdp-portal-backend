...
public class EcrEventListenerTest
{
    ...
    [Fact]
    public async Task TestSemVerV2PushMessage()
    {
        var message = """
                      {
                          "version": "0",
        ...
        await _artifacts
            .Received()
            .CreateAsync(
                Arg.Is<DeployableArtifact>(d =>
                    d.Repo == "my-repository-name" &&
                    d.Tag == "0.1.0-rc.1+build.1" &&
                    d.Sha256 == "sha256:7f5b2640fe6fb4f46592dfd3410c4a79dac4f89e4782432e0378abcd1234"),
                Arg.Any<CancellationToken>());
        await _autoDeploymentTriggerExecutor.Received()
            .Handle(Arg.Is("my-repository-name"), Arg.Is("0.1.0-rc.1+build.1"), Arg.Any<CancellationToken>());
    }
